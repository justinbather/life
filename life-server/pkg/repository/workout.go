...
func (r *workoutRepository) CreateWorkout(ctx context.Context, workout model.Workout) (model.Workout, error) {
	res, err := r.queries.CreateWorkout(ctx, sqlc.CreateWorkoutParams{Type: workout.Type, UserID: workout.User, Duration: int32(workout.Duration), CaloriesBurned: int32(workout.CaloriesBurned), Workload: int32(workout.Workload), Description: &workout.Description})
	if err != nil {
		r.logger.Errorf("Error in WorkoutRepository.CreateWorkout: %s", err)
		return model.Workout{}, err
	}

	return mapWorkoutRow(res), nil
}

func (r *workoutRepository) GetWorkoutsByType(ctx context.Context, user, workoutType string) ([]model.Workout, error) {
	records, err := r.queries.GetWorkoutsByType(ctx, sqlc.GetWorkoutsByTypeParams{UserID: user, Type: workoutType})
	if err != nil {
...
func (r *workoutRepository) GetWorkoutsFromDateRange(ctx context.Context, user string, from, to time.Time) ([]model.Workout, error) {
	records, err := r.queries.GetWorkoutsFromDateRange(ctx, sqlc.GetWorkoutsFromDateRangeParams{UserID: user, CreatedAt: mapDate(from), CreatedAt_2: mapDate(to)})
	if err != nil {
...
func mapWorkout(w sqlc.Workout) model.Workout {
	return model.Workout{Id: int(w.ID), User: w.UserID, Type: w.Type, Duration: int(w.Duration), CreatedAt: w.CreatedAt.Time, Workload: int(w.Workload), CaloriesBurned: int(w.CaloriesBurned), Description: *w.Description}
}

func mapWorkoutRow(r sqlc.CreateWorkoutRow) model.Workout {
	return model.Workout{Id: int(r.ID), User: r.UserID, Type: r.Type, Duration: int(r.Duration), CreatedAt: r.CreatedAt.Time, Workload: int(r.Workload),
		CaloriesBurned: int(r.CaloriesBurned), Description: *r.Description}
}
